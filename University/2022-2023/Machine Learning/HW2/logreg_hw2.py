...
def main():
  ...
  X_test_bias = dummyAugment(X_test_bias)
  y_pred_test = X_test_bias@w >= 0

  with open("test-prediction1.csv", 'w+') as file:
    for i in range(len(y_pred_test)):
      s = str(i) + "," + str(int(y_pred_test[i])) +"\n"
      file.write(s)
...
def logistic(z):
  ...
  for i in range(0, z.size):
    logit_z.append(L / (1 + math.exp(-k*(z[i] - center))))
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  for i in range(1, y.size):
    logit = logistic(np.transpose(w) @ X[i])
    nll += y[i] * log(logit[0] + 0.0000001) + (1 - y[i]) * log(1- logit[0] + 0.0000001)
