...
def main():
  ...
  with open("prediction.csv", 'w+') as file:
    ...
    while i < len(X_test):
      ...
      logit = net.forward(X_test[i])
      estimate = np.argmax(logit)
      s = str(i) + "," + str(estimate) +"\n"
      file.write(s)
      i = i + 1
...
class LinearLayer:
  ...
  def backward(self, grad):
    # grad is dL/dZ
    self.grad_weights = np.transpose(self.input)@grad # Compute dL/dW as in Eq. 32
    self.grad_bias = np.sum(grad[0]) # Compute dL/db as in Eq. 27
    
    return grad@np.transpose(self.weights) # Compute dL/dX as in Eq. 35
