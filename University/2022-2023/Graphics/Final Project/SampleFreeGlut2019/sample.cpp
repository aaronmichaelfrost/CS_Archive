...
unsigned char* sandTexture;
unsigned char* sunTexture;
int waterTextureWidth;
int waterTextureHeight;

int sandTextureHeight;
int sandTextureWidth;

int sunTextureHeight;
int sunTextureWidth;
...
void
OsuWaveSphere(float radius, int slices, int stacks, float time)
{
	// set the globals:
	...
	for (int ilat = 0; ilat < SphNumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < SphNumLngs; ilng++)				// ilng=0, lng=-M_PI and
											// ilng=SphNumLngs-1, lng=+M_PI are the same meridian
		{
			float r = radius + sin(ilat) / 100 * sin(time);
			...
			p->ny = y;
			p->nz = z;
			p->s = (lng + M_PI) / (2. * M_PI) * cos(time / 100) ;
			p->t = (lat + M_PI / 2.) / M_PI * cos(time / 100);
		}
	}
...
Animate( )
{
	...
	Time = glutGet(GLUT_ELAPSED_TIME) / (float)1000;

	movingX = sin(float(ms) / (float)1000);
...
Display( )
{
	...
	glColor3f(0, 1, 0);
	glTranslatef((GLfloat)movingX, (GLfloat)2, (GLfloat)0);
	OsuSphere(1, 100, 100);
	glPopMatrix();*/
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, sunTextureWidth, sunTextureHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, sunTexture);
	glMatrixMode(GL_TEXTURE);
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();

	// rotate around sun
	SetPointLight(GL_LIGHT0, (GLfloat)orbitDistance * cos(Time / 10), 0, (GLfloat)orbitDistance * sin(Time / 10), 1, 1, .5);
	glTranslatef((GLfloat)orbitDistance * cos(Time / 10), 0, (GLfloat)orbitDistance * sin(Time / 10));
	...
	// rotate self axis
	glRotatef((GLfloat)glutGet(GLUT_ELAPSED_TIME) / 10, (GLfloat)0, (GLfloat)1, (GLfloat)0);

	

	// Draw sandy islands
	glTexImage2D(GL_TEXTURE_2D, 0, 3, sandTextureWidth, sandTextureHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, sandTexture);
	glMatrixMode(GL_TEXTURE);
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	SetMaterial(1., 1., 1., 0.);
	glColor3f(1, 1, 1);
	glTranslatef((GLfloat)0, (GLfloat)0, (GLfloat).357);
	OsuSphere(.4, 100, 100);
	glPopMatrix();

	glPushMatrix();
	glTranslatef((GLfloat).21, (GLfloat).188, (GLfloat).207);
	OsuSphere(.4, 100, 100);
	glPopMatrix();

	glPushMatrix();
	glTranslatef((GLfloat).275, (GLfloat).188, (GLfloat).114);
	OsuSphere(.4, 100, 100);
	glPopMatrix();

	glPushMatrix();
	glTranslatef((GLfloat).134, (GLfloat).188, (GLfloat)-.286);
	OsuSphere(.4, 100, 100);
	glPopMatrix();

	glPushMatrix();
	glTranslatef((GLfloat)-.136, (GLfloat)-.138, (GLfloat)-.282);
	OsuSphere(.4, 100, 100);
	glPopMatrix();
	...
	glRotatef((GLfloat)glutGet(GLUT_ELAPSED_TIME) / 100, (GLfloat)0, (GLfloat)1, (GLfloat)0);
	OsuSphere(.65, 100, 100);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, waterTextureWidth, waterTextureHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, waterTexture);
	// Draw Water sphere
	glMatrixMode(GL_TEXTURE);
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	SetMaterial(1., 1., 1., 0., .7);

	glTranslatef((GLfloat)0, (GLfloat)0, (GLfloat)0);
	glRotatef((GLfloat)glutGet(GLUT_ELAPSED_TIME) / 100, (GLfloat)0, (GLfloat)1, (GLfloat)0);
	OsuWaveSphere(.7, 100, 100, movingX * 10);
	SetMaterial(1., 1., 1., 0., .7);
	glPopMatrix();
...
InitGraphics( )
{
	...
	waterTexture = BmpToTexture("water.bmp", &waterTextureWidth, &waterTextureHeight); 
	sandTexture = BmpToTexture("sand.bmp", &sandTextureWidth, &sandTextureHeight);
	sunTexture = BmpToTexture("8k_sun.bmp", &sunTextureWidth, &sunTextureHeight);
}
...
void
SetMaterial(float r, float g, float b, float shininess, float a)
{
