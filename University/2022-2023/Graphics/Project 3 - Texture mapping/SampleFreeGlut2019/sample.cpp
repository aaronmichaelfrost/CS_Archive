...
TextureMode textureMode = DEFAULT;

float distortAmount = 0;
float distortAmount2 = 0;
...
OsuCone(float radBot, float radTop, float height, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < ConeNumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < ConeNumLngs; ilng++)
		{
			...
			if (textureMode == DISTORTED) 
			{
				p->s = (float)ilng / (float)(ConeNumLngs - 1) * p->t * cos(distortAmount2) - .5;
				p->t /= cos(distortAmount);
			}
		}
	}
...
Animate( )
{
	...
	distortAmount = cos(((float)ms * 2) / (float)(5000));
	distortAmount2 = sin((float)ms * 10 / (float)(5000));
...
Display( )
{
	...
	if (textureMode != DEFAULT) 
	{
		if (textureMode == TEXTURED)
		{
			glutIdleFunc(NULL);
		}
		else if (textureMode == DISTORTED)
		{
...
InitGraphics( )
{
	...
	texture = BmpToTexture("spiral.bmp", &textureWidth, &textureHeight);
}
