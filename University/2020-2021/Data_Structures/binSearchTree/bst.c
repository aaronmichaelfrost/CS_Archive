...
struct bst_node* create_new_node(int key, void* value); 
...
int bst_size(struct bst* bst) {
  ...
  if(bst->root == NULL)
    ...
    struct bst left_bst;
    struct bst right_bst;

    struct bst* l = &left_bst;
    struct bst* r = &right_bst;

    l->root = bst->root->left;
    r->root = bst->root->right;

    return bst_size(l) + bst_size(r) + 1;
  }
...
void bst_insert(struct bst* bst, int key, void* value) {
  ...
  if(bst->root == NULL)
  {
    bst->root = create_new_node(key, value);
  }
  else{
    ...
    if(key < p->key)
      p->left = create_new_node(key, value);
    else
      p->right = create_new_node(key, value);
...
void bst_remove(struct bst* bst, int key) {
  ...
  if(temp){
    ...
    // If it has a left subtree, connect it to the parent
    ...
    // If it has a right subtree, connect it to the parent
    else if (temp->right && temp->left == NULL){
    ...
    // If it has two children, connect the parent to the successor
    else{

      struct bst_node* successor = temp->right;
      struct bst_node* successor_parent = temp;
