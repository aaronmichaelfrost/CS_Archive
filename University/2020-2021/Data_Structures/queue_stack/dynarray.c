...
int dynarray_set_size(struct dynarray * da, int size){
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  int i = da->logical_start;

  for (int x = 0; x < da->size;)
  {
    if(i == da->capacity)
      i = 0;

    new_data[x++] = da->data[i++];
  }
...
void dynarray_insert(struct dynarray* da, void* val) {
  ...
  // Put the new element at the logical end of the array.
  da->data[da->size] = val;
  da->size++;
...
void dynarray_circular_insert(struct dynarray* da, void* val) {
  assert(da);
  ...
  {
    _dynarray_resize(da, 2 * da->capacity);
    da->logical_end = da->size;
  }

  // Wrap the end to the start
  if(da->logical_end == da->capacity && da->logical_start > 0)
      da->logical_end = 0;

  // Put the new element at the logical end of the array.
  da->data[da->logical_end++] = val;
  da->size++;
...
int dynarray_get_logicalstart(struct dynarray* da){
  return da->logical_start;
}


void dynarray_set_logicalstart(struct dynarray* da, int val){
  da->logical_start = val;
}
